...
code = code.replace(
    ...
    `<button data-action="\${clientData ? \`client:update:\${clientData.name}\` : 'client:register'}" class="$1">\${clientData ? 'Update Client' : 'Save Client'}</button>`
).replace(
    ...
    /id="clientAddress" rows="3" class="(.*?)"><\/textarea>/,
    `id="clientAddress" rows="3" class="$1">\${esc(clientData?.address || '')}</textarea>`
);


// Note: Since you can't be sure about ProjectRegistration exactly, let's inject handlers in handleAction using a separate block
let handleActionIndex = code.indexOf('                    const msg = btn.getAttribute(\'data-toast\');');
if (handleActionIndex !== -1) {
    // wait handleAction is inside setupEventListeners or handleAction function.
}
